...
<?php
	...
	//Load LOAISANPHAM
	$loaiSP=$conenct->prepare('SELECT MALOAISP FROM `loaisanpham`');
    $loaiSP->execute();
    $loaiSPData = $loaiSP->fetchALL();

	 //RAM
	$result=$conenct->prepare('SELECT TENSP, GIA_BAN, TENFILE,MASP FROM sanpham, khohinh WHERE MASP = MADOITUONG AND GHICHU = "Cover" AND MALOAI = "RAM" limit 3');
    $result->execute();
    ...
    $rowsdata2 = $result2->fetchALL();
	 //GPU
	$result3=$conenct->prepare('SELECT TENSP, GIA_BAN, TENFILE,MASP FROM sanpham, khohinh WHERE MASP = MADOITUONG AND GHICHU = "Cover" AND MALOAI = "GPU" limit 3');
    $result3->execute();
...
<style> 
	...
	men-thumb-item text-center.img{
		width:100%; 
		height:100%;
	} 	
		...
		<div class="container py-xl-4 py-lg-2">
			...
			<div class="row">
				...
				<div class="agileinfo-ads-display col-lg-9">
					<div class="wrapper">
						<!-- first section -->
						<?php foreach($loaiSPData as $loai){
							$sanPham=$conenct->prepare('SELECT TENSP, GIA_BAN, TENFILE,MASP FROM sanpham, khohinh WHERE MASP = MADOITUONG AND GHICHU = "Cover" AND MALOAI = "'.$loai['MALOAISP'].'" ');
							$sanPham->execute();
							$sanPhamRows = $sanPham->fetchALL();
							$sanPhamCount = $sanPham->rowCount();
							if ($sanPhamCount>0){

							
							?>
								<div class="product-sec1 px-sm-4 px-3 py-sm-5  py-3 mb-4">
								<h3 class="heading-tittle text-center font-italic"><?php echo$loai['MALOAISP']?></h3>
							<div class="row">
							<?php
								foreach($sanPhamRows as $i){
								?>
